...
def run_algo_comparison():
    ...
    header = (
        f"{'Algorithm':<12} | {'Needle (S/R)':<15} | {'QA (S/R)':<15} | {'Coherence (S/R)':<15}"
    )
    print(header)
    print("-" * len(header))
    ...
    for algo, res in results.items():
        ...
        c = res["Coherence"]
        print(
            f"{algo:<12} | {n['savings']:>4.1f}%/{n['recall']:>3.0f}% | {q['savings']:>4.1f}%/{q['recall']:>3.0f}% | {c['savings']:>4.1f}%/{c['recall']:>3.0f}%"
        )
