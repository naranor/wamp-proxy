...
def test_hypothesis_priority_needle():
    ...
    for category, queries in test_suite.items():
        for query in queries:
            ...
            p_needle = 1 / (1 + np.exp(-(np.dot(np.array(nw["coef"]), features) + nw["intercept"])[0]))
            p_reason = 1 / (1 + np.exp(-(np.dot(np.array(rw["coef"]), features) + rw["intercept"])[0]))
            ...
            print(f"{query[:35]:<35} | {p_needle:.4f} | {p_reason:.4f} | {predicted:<10} | {'✅' if success else '❌'}")
