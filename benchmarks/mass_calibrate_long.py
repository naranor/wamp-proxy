...
def run_mass_calibration():
    ...
    for name, data in scenarios.items():
        ...
        for mult in multipliers:
            ...
            kept_indices = [i for i, s in enumerate(scores) if i == 0 or i >= len(msgs)-2 or s >= threshold]
            ...
            if recall == 0: status = "❌ FAILED"
            
            print(f"  Mult {mult:.2f}: Savings {savings:>4.1f}% | {status} | Kept {len(kept_indices)}/{len(msgs)}")
