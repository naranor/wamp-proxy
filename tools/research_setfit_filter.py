...
def run_setfit_research():
        ...
        for algo in algorithms:
            for mult in multipliers:
                ...
                for i, msg in enumerate(msgs):
                    if i == 0 or i >= len(msgs) - 2: continue # Skip system/last
                    ...
                    if len(input_ids) > 1024: input_ids = input_ids[:1024]
                    
                    ids_np = np.array([input_ids], dtype=np.int64)
                    mask_np = np.ones_like(ids_np)

                    outputs = pruner.session.run(None, {'input_ids': ids_np, 'attention_mask': mask_np})
                    ...
                    for layer_attn in layers:
                        ...
                        if algo == "mean_max": importance += float(np.mean(np.max(slc, axis=2)))
                        elif algo == "cls_max": importance += float(np.max(slc[:, 0, :]))
                ...
                results.append({
                    "Scenario": name, "Algo": algo, "Mult": mult, 
                    "Sav": savings, "Rec": recall, "Status": status
                })
                print(f"[{status}] {algo:<10} | mult={mult:.2f} | Sav={savings:>5.1f}% | Rec={recall:>3.0f}%")
    ...
    with open("benchmarks/RESEARCH_SETFIT_FILTER.md", "w") as f:
        ...
        for name in scenarios.keys():
            ...
            for r in results:
                if r["Scenario"] == name:
                    f.write(f"| {r['Algo']} | {r['Mult']:.2f} | {r['Sav']:.1f}% | {r['Rec']:.0f}% | {r['Status']} |\n")
            f.write("\n")
