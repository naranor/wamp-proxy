...
def generate_massive_dataset():
    ...
    needle_bases = ["port", "api key", "password", "secret", "ip address", "budget", "version", "date", "token", "string", "constant", "value", "id", "key", "email", "name"]
    needle_phrases = [
        "What is the {x}?", "Find the {x}.", "Tell me the {x}.", "Show me the {x}.", "Retrieve the {x}.",
        "Give me the {x} mentioned earlier.", "What was the {x} from before?", "Identify the {x} in the history.",
        "Could you find the {x} we discussed?", "I need the {x} value.", "{x}?", "What is the {x} number?",
        "Where is the {x}?", "Search for the {x}.", "Extract the {x}."
    ]
    
    # Templates for Reasoning
    reasoning_bases = ["Alpha and Beta", "Kong and OAuth2", "monolith and microservices", "ELK and Prometheus", "budget and performance", "current and future state"]
    reasoning_phrases = [
        "Compare {x}.", "Analyze the relationship between {x}.", "Why did {x} happen?", "Evaluate the {x}.",
        "Contrast {x}.", "Synthesize the data for {x}.", "What are the differences in {x}?",
        "Explain the logic of {x}.", "Cross-reference {x}.", "Based on logs, why {x}?",
        "Justify the use of {x}.", "Critique the design of {x}."
    ]
    
    # Templates for Summary
    summary_phrases = [
        "Summarize our talk.", "TL;DR.", "Give a brief overview.", "Concise report.", "Short synopsis.",
        "Recap the main points.", "High-level summary.", "Condensed version.", "Wrap up the chat.",
        "General topic of conversation?", "Briefly outline the plan.", "Technical synopsis.",
        "What have we discussed so far?", "One paragraph summary please.", "Synopsis of the history."
    ]
...
def train_massive_router():
    ...
    clf_n = LogisticRegression(max_iter=5000, C=0.5, class_weight='balanced').fit(np.array(X), np.array(y))
    ...
    clf_r = LogisticRegression(max_iter=5000, C=0.5, class_weight='balanced').fit(np.array(X), np.array(y))
