...
def train_reasoning_v2():
    ...
    others = [
        ...
        "Show me the secret key.", "What is the backup secret?", "Find server IP.",
        "DB connection string.", "What's the admin pass?", "Find lead scientist name."
    ] * 5 
    
    X, y = [], []
    print("Extracting V2 composite features...")
    for q in reasoning: X.append(get_composite_features(pruner, q)); y.append(1)
    for q in others: X.append(get_composite_features(pruner, q)); y.append(0)
            
    clf = LogisticRegression(max_iter=5000, C=1.0, class_weight='balanced').fit(np.array(X), np.array(y))
