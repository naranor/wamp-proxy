...
def export_model(model_id, output_dir, task="feature-extraction"):
    ...
    if not fp32_path.exists():
        print(f"Step 1: Exporting to FP32 (attentions enabled)...")
        ...
        class CustomOnnxConfig(onnx_config.__class__):
            ...
            def outputs(self) -> OrderedDict:
                ...
                for i in range(layers):
                    outputs[f"attentions.{i}"] = {0: "batch", 1: "num_heads", 2: "sequence", 3: "sequence"}
                return outputs
    ...
    if fp32_path.exists() and not int8_path.exists():
        print(f"Step 2: Quantizing to INT8 (AVX-512 VNNI optimized)...")
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Export Transformer models to ONNX with Attention Weights")
    parser.add_argument("--model", type=str, required=True, help="HuggingFace model ID")
    parser.add_argument("--out", type=str, required=True, help="Output directory")
    parser.add_argument("--task", type=str, default="feature-extraction", help="Task type (feature-extraction or text-classification)")
