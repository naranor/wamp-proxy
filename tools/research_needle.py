...
def run_step_2_needle():
    print("=== STEP 2: NEEDLE IN A HAYSTACK RESEARCH ===")

    pruner = WAMPruner(model_dir="./deberta_nli_model")

    # 112 messages scenario
    msgs = generate_haystack(needle_idx=25, total_msgs=55)
    task_query = "What was the database port mentioned earlier?"
    ...
    critical_indices = [51, 52]  # Fact + confirmation
    ...
    for algo in algorithms:
        ...
        # Patch the batch processor for the specific algorithm
        def custom_batch(self, batch_data, task_ids, task_len, scores_dict):
            input_ids = np.zeros(
                (len(batch_data), task_len + max(i["len"] for i in batch_data)), dtype=np.int64
            )
        ...
        # Pre-calculate scores (baseline stays same for given algo)
        ...
        for mult in multipliers:
            ...
            recall = (
                len([idx for idx in critical_indices if idx in kept]) / len(critical_indices) * 100
            )
            savings = (1 - len(kept) / len(msgs)) * 100
            results[algo].append({"mult": mult, "savings": savings, "recall": recall})

    # Generate Markdown Table
    with open("benchmarks/RESEARCH_NEEDLE.md", "w") as f:
        f.write("# Research Step 2: Needle In A Haystack Audit (112 msgs)\n")
        f.write("**Model:** DeBERTa-v3-small-NLI (INT8)\n\n")

        for algo in algorithms:
            f.write(f"### Algorithm: {algo}\n")
            f.write("| Multiplier | Compression % | Recall (Fact) % | Status |\n")
            f.write("| :--- | :--- | :--- | :--- |\n")
            for res in results[algo]:
                status = (
                    "✅ PASS"
                    if res["recall"] == 100
                    else ("⚠️ PARTIAL" if res["recall"] > 0 else "❌ FAIL")
                )
                f.write(
                    f"| {res['mult']:.2f} | {res['savings']:.1f}% | {res['recall']:.0f}% | {status} |\n"
                )
            f.write("\n")

    print("🚀 Research results saved to benchmarks/RESEARCH_NEEDLE.md")
