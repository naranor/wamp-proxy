...
def get_v1_features(pruner, text):
    ...
    return np.concatenate([last_hidden[0], np.mean(last_hidden, axis=0), [kurtosis(np.mean(outputs[-1][0], axis=0).flatten()), np.max(outputs[-1][0]), len(encoding.ids)]])
...
def get_v4_features(pruner, text):
    ...
    return np.concatenate([cls_emb, mean_emb, delta_emb, [np.linalg.norm(delta_emb), kurtosis(flat_attn), np.max(flat_attn), np.std(flat_attn), len(encoding.ids), 0.0, 0.0]])
...
def run_accuracy_audit():
    ...
    for cat_name, queries in ds.items():
        ...
        for q in queries:
            ...
            v1_pred = "Reasoning" if p_r1 > 0.5 else ("Needle" if sigmoid(np.dot(v1_nw["coef"], f1) + v1_nw["intercept"])[0] > 0.5 else "Summary")
